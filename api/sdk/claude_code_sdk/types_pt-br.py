...
# Permission atualizar types (matching TypeScript SDK)
...
# #### Hook types
# suportado hook event types. Due para Configuração limitations, the Python SDK does não
# support SessionStart, SessionEnd, e Notification hooks.
HookEvent = Literal[
    "PreToolUse",
    "PostToolUse",
    "UserPromptSubmit",
...
# See https://docs.anthropic.com/en/docs/claude-code/hooks#avançado%3A-json-saída
# for Documentação of the saída types. atualmente, "continue", "stopReason",
# e "suppressOutput" are não suportado in the Python SDK.
class HookJSONOutput(TypedDict):
    # Whether para block the action related para the hook.
    decision: NotRequired[Literal["block"]]
    # Optionally adicionar a system message aquele is não visível para Claude but saved in
    # the chat transcript.
    systemMessage: NotRequired[str]
    # See each hook's individual "Decision Control" section in the Documentação
    # for guidance.
...
HookCallback = Callable[
    # HookCallback entrada parâmetros:
    # - entrada
    # See https://docs.anthropic.com/en/docs/claude-code/hooks#hook-entrada for
    # the type of 'entrada', the primeiro value.
    # - tool_use_id
    # - context
...
class HookMatcher:
    ...
    # a tool name like "Bash" ou a combination of tool names like
    # "escrever|MultiEdit|editar".
    matcher: Optional[str] = None

    # A lista of Python funções com função signature HookCallback
...
class SDKControlPermissionRequest(TypedDict):
    ...
    tool_name: str
    input: Dict[str, Any]
    # TODO: adicionar PermissionUpdate type aqui
