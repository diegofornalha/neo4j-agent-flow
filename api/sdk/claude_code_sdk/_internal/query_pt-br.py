...
class Query:
    ...
    async def initialize(self) -> Optional[Dict[str, Any]]:
        ...
        # construir hooks Configuração for initialization
        ...
        try:
            async for message in self.transport.read_messages():
                ...
                if msg_type == "control_response":
                    ...
                    # Handle incoming control requests de CLI
                    # Cast message para SDKControlRequest for type safety
                    ...
                    # Handle cancelar requests
                    # TODO: Implement cancellation support
                    continue

                # Regular SDK messages go para the stream
                await self._message_send.send(message)

        except anyio.get_cancelled_exc_class():
            # tarefa was cancelled - este is expected behavior
            logger.debug("Read task cancelled")
            raise  # Re-raise to properly handle cancellation
        except Exception as e:
            logger.error(f"Fatal error in message reader: {e}")
            # Put erro in stream so iterators can handle it
            await self._message_send.send({"type": "error", "error": str(e)})
        finally:
            # sempre signal end of stream
    ...
    async def _handle_control_request(self, request: SDKControlRequest) -> None:
        ...
        try:
            ...
            if subtype == "can_use_tool":
                ...
                # Handle tool permission requisição
                ...
                # Convert PermissionResult para expected dict format
                if isinstance(response, PermissionResultAllow):
                    response_data = {"allow": True}
                    if response.updated_input is not None:
                        response_data["input"] = response.updated_input
                    # TODO: Handle updatedPermissions quando control protocol supports it
                elif isinstance(response, PermissionResultDeny):
                    response_data = {"allow": False, "reason": response.message}
                    # TODO: Handle interrupt flag quando control protocol supports it
                else:
                    raise TypeError(
    ...
    async def _send_control_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate unique requisição ID
        self._request_counter += 1
        request_id = f"req_{self._request_counter}_{os.urandom(4).hex()}"

        # criar event for resposta
        event = anyio.Event()
    ...
    async def _handle_sdk_mcp_request(
        ...
        try:
            # TODO: Python MCP SDK lacks the Transport abstraction aquele TypeScript has.
            # TypeScript: servidor.conectar(transport) allows personalizado transports
            # Python: servidor.executar(read_stream, write_stream) requires actual streams
            #
            # este forces us para manually route métodos. quando Python MCP adds Transport
            # support, we can refactor para match the TypeScript approach.
            if method == "initialize":
                ...
                if handler:
                    ...
                    # Convert MCP resultado para JSONRPC resposta
                    tools_data = [
                        {
                            "name": tool.name,
            ...
            # adicionar mais métodos aqui as MCP SDK adds them (resources, prompts, etc.)
            # este is the limitation Ashwin pointed out - we have para manually atualizar
    ...
    async def stream_input(self, stream: AsyncIterable[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            # depois todos messages sent, end entrada
    ...
    async def receive_messages(self) -> AsyncIterator[Dict[str, Any]]:
        ...
        async for message in self._message_receive:
            # verificar for special messages
    ...
    async def close(self) -> None:
        ...
        if self._tg:
            ...
            # Wait for tarefa grupo para completo cancellation
