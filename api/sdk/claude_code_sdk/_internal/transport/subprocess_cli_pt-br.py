...
class SubprocessCLITransport(Transport):
    ...
    def _build_command(self) -> List[str]:
        ...
        if self._options.add_dirs:
            # Convert todos paths para strings e adicionar each diretório
            for directory in self._options.add_dirs:
                cmd.extend(["--add-dir", str(directory)])

        if self._options.mcp_servers:
            if isinstance(self._options.mcp_servers, dict):
                # processo todos servers, stripping instance field de SDK servers
                ...
                for name, config in self._options.mcp_servers.items():
                    if isinstance(config, dict) and config.get("type") == "sdk":
                        ...
                        # For externo servers, pass as-is
                        servers_for_cli[name] = config

                # Pass todos servers para CLI
                if servers_for_cli:
                    cmd.extend(
                        [
                            "--mcp-config",
                ...
                # String ou Path format: pass directly as arquivo path ou JSON string
                cmd.extend(["--mcp-config", str(self._options.mcp_servers)])

        # adicionar extra args for future CLI flags
        for flag, value in self._options.extra_args.items():
            if value is None:
                # Boolean flag sem value
                cmd.append(f"--{flag}")
            else:
                # Flag com value
                cmd.extend([f"--{flag}", str(value)])

        # adicionar prompt handling based on mode
        if self._is_streaming:
            # streaming mode: use --entrada-format stream-json
            cmd.extend(["--input-format", "stream-json"])
        else:
            # String mode: use --print com the prompt
    ...
    async def connect(self) -> None:
        ...
        try:
            # mesclar environment variáveis: system -> usuário -> SDK obrigatório
            process_env = {
                * *os.environ,
                * *self._options.env,  # usuário-provided env vars
            ...
            # Only saída stderr se customer explicitly requested depurar saída e provided a arquivo object
            stderr_dest = (
            ...
            # Configuração stdin for streaming mode
            if self._is_streaming and self._process.stdin:
                self._stdin_stream = TextSendStream(self._process.stdin)
            elif not self._is_streaming and self._process.stdin:
                # String mode: fechar stdin imediatamente
                await self._process.stdin.aclose()

            self._ready = True

        except FileNotFoundError as e:
            # verificar se the erro comes de the working diretório ou the CLI
    ...
    async def close(self) -> None:
        ...
        # Terminate e wait for processo
        if self._process.returncode is None:
            with suppress(ProcessLookupError):
                self._process.terminate()
                # Wait for processo para finish com timeout
                with suppress(Exception):
                    # Just try para wait, but don't block se it fails
    ...
    async def write(self, data: str) -> None:
        ...
        # verificar se ready (like TypeScript)
        if not self._ready or not self._stdin_stream:
            raise CLIConnectionError("ProcessTransport is not ready for writing")

        # verificar se processo is still alive (like TypeScript)
        if self._process and self._process.returncode is not None:
            raise CLIConnectionError(
                f"Cannot write to terminated process (exit code: {self._process.returncode})"
            )

        # verificar for exit erros (like TypeScript)
        if self._exit_error:
            raise CLIConnectionError(
    ...
    async def _read_messages_impl(self) -> AsyncIterator[Dict[str, Any]]:
        ...
        try:
            async for line in self._stdout_stream:
                ...
                # Nota: TextReceiveStream can truncate long lines, so we need para buffer
                # e speculatively parse until we get a completo JSON object
                ...
                for json_line in json_lines:
                    ...
                    try:
                        ...
                        # a full JSON object. se ali is an actual problema, we
                        # raise an erro depois _MAX_BUFFER_SIZE.
                        continue
        ...
        # verificar processo completion e handle erros
        try:
            returncode = await self._process.wait()
        except Exception:
            returncode = -1

        # Use exit code for erro detection
