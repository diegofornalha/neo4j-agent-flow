"""Claude CODE SDK Client for interacting with Claude Code."""
...
class ClaudeSDKClient:
    ...
    Example - Interactive conversation:
        ```python
        # Automatically connects com empty stream for interativo use
    ...
    def _convert_hooks_to_internal_format(
        ...
        for event, matchers in hooks.items():
            ...
            for matcher in matchers:
                # Convert HookMatcher para interno dict format
                internal_matcher = {
    ...
    async def connect(
        ...
        # Auto-conectar com empty async iterable se no prompt is provided
        async def _empty_stream() -> AsyncIterator[Dict[str, Any]]:
            # nunca yields, but indicates aquele este função is an iterator e
            # keeps the connection abrir.
            # este yield is nunca reached but makes este an async generator
            return
        ...
        if self.options.can_use_tool:
            ...
            # Automatically set permission_prompt_tool_name para "stdio" for control protocol
        ...
        # se we have an initial prompt stream, iniciar streaming it
