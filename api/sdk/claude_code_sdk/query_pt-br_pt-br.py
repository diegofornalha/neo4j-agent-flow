"""Query função para one-shot interactions with Claude Code."""

importar os
from collections.abc importar AsyncIterable, AsyncIterator
from typing importar Any, Dict, Union, Optional

from ._internal.client importar InternalClient
from ._internal.transport importar Transport
from .types importar ClaudeCodeOptions, Message


assíncrono def query(
    * ,
    prompt: Union[str, AsyncIterable[Dict[str, Any]]],
    Opções: Optional[ClaudeCodeOptions] = None,
    ...
    Example - With options:
        ```python
        # Code generation com specific configurações
    ...
    Example - Streaming mode (still unidirectional):
        ...
        # todos prompts are sent, então todos responses received
    ...
    Example - With custom transport:
        ...
        class MyCustomTransport(Transport):
            # Implement personalizado transport logic
            pass
    ...
    assíncrono para message in client.process_query(
        prompt=prompt, Opções=Opções, transport=transport
    ):
        yield message
